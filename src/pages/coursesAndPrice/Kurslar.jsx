...
const Kurslar = () => {
  ...
  // ? course
  const { data: courseTitle } = useFatch('apicourses')
  const { data: coursesAndPriceData, isLoading, error } = useFatch('apimentors/', 'testinCourses')
        ...
        <div className='max-width'>
          <div className='kurslar'>
            ...
            <div className='between'>
              {/* Har bir mentor alohida card */}
              {coursesAndPriceData?.map((mentor, index) => {
                const firstCourse = mentor?.courses?.[0]

                return (
                  <div className='between_fist_div' key={mentor.id}>
                    {/* Chap qism */}
                    <h1 className='qoshimcha-kurs-nomi'>
                      {mentor.description.slice(0, -12)}
                    </h1>
                    ...
                    <div className='grid-item min_div'>
                      ...
                      <div className='min_div_none'>
                        ...
                        <p className='font-size-18-500'>{mentor.description}</p>
                      </div>
                    </div>
                      ...
                      <div className='grid_bar'>
                        {mentor?.courses?.map(course => (
                          <React.Fragment key={course.id}>
                            <div className='month'>
                              <span className='font-size-20'>Kurs davomiyligi</span>
                              <p className='font-size-20'>{course.duration_months} oy</p>
                            </div>

                            <div className='time'>
                              <span className='font-size-20'>Dars boshlanish soati</span>
                              <p className='font-size-20'>-------</p>
                            </div>

                            <div className='times'>
                              <span className='font-size-20'>Haftada</span>
                              <p className='font-size-20'>{course.weekly_hours} kun</p>
                            </div>

                            <div className='clock'>
                              <span className='font-size-20'>Necha soat</span>
                              <p className='font-size-20'>{course.duration_hours} soat</p>
                            </div>
                          </React.Fragment>
                        ))}
                      </div>
